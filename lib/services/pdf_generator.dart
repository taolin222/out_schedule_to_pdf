...
class PdfGenerator {
  ...
  static Future<Uint8List> generatePdf(StudyPlan plan) async {
    ...
    final fontData = await rootBundle.load('assets/fonts/NotoSansSC-Medium.ttf');
  ...
  static pw.Widget _buildHeader(StudyPlan plan, pw.Font font) {
    ...
    return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.center, children: [
      ...
      pw.Row(mainAxisAlignment: pw.MainAxisAlignment.center, children: [
        pw.Text('${plan.planDate.year}年 ${plan.planDate.month}月 ${plan.planDate.day}日 ${plan.weekdayChinese}    学习时长：',
            style: pw.TextStyle(font: font, fontSize: _infoFontSize, fontWeight: pw.FontWeight.bold)),
        pw.Container(width: 80, height: _infoFontSize + 2,
          decoration: const pw.BoxDecoration(
            border: pw.Border(bottom: pw.BorderSide(color: PdfColors.black, width: 1)),
          ),
        ),
        pw.SizedBox(width: 4),
        pw.Text('  $examText', style: pw.TextStyle(font: font, fontSize: _infoFontSize, fontWeight: pw.FontWeight.bold)),
      ]),
    ]);
  }
  ...
  static pw.TableRow _makeRow(List<String> texts, pw.Font font,
      {double height = 38, bool bottomBorder = true,
      bool mergeCol0 = false, bool mergeCol1 = false,
      bool mergeCol1Right = false, bool mergeCol2Right = false,
      int leftAlignCol = -1}) {
    ...
    final cells = List.generate(8, (i) {
      ...
      return _borderedCell(texts[i], font, height: height,
  ...
  static pw.Widget _buildTable(StudyPlan plan, pw.Font font) {
    ...
    // 数量（固定2行）
    rows.add(_makeRow([_catChar('数量', 0), '', '', '', '', '', '', ''], font,
        height: rh, bottomBorder: true, mergeCol0: true, mergeCol1Right: true));
    rows.add(_makeRow([_catChar('数量', 1), '', '', '', '', '', '', ''], font,
        height: rh, bottomBorder: true, mergeCol1Right: true));

    // 资料分析（固定2行）
    rows.add(_makeRow([_catChar('资料分析', 0), '', '', '', '', '', '', ''], font,
        height: rh, bottomBorder: true, mergeCol0: true, mergeCol1Right: true));
    rows.add(_makeRow([_catChar('资料分析', 1), '', '', '', '', '', '', ''], font,
        height: rh, bottomBorder: true, mergeCol1Right: true));
